...
class MessagesProcessor {
    ...
    fun getMessageAnswer(message: ReceivedMessage, history: List<Persister.ChatHistoryEntry>): MessageAnswer? {
        ...
        val messageWithoutPunctuation = message.body.replace(",", "").replace(".", "").replace("?", "")

        val answer = run {
            if (history.isEmpty()) {
                if (messageProcessorData.initialMessages.any { collator.compare(it, messageWithoutPunctuation) == 0 }) {
                ...
                if (
                    ...
                    messageProcessorData.finishMessageAndFinishAnswer!!.first.isNotEmpty() &&
                    collator.compare(messageWithoutPunctuation, messageProcessorData.finishMessageAndFinishAnswer!!.first) == 0
                ) {
                ...
                if (
                    ...
                    collator.compare(messageWithoutPunctuation, messageProcessorData.goBackMessage) == 0 &&
