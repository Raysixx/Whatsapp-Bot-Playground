...
object WhatsAppController {
    ...
    object ActiveChatsRequestHandler: HttpHandler {
        override fun invoke(request: Request) = Response(OK).body(historyControl.getActiveChatsCount().toString())
    }

    val handler: HttpHandler = routes(
        "whatsapp" bind GET to RequestHandler,
        "whatsapp" bind POST to RequestHandler,
        "active-chats" bind GET to ActiveChatsRequestHandler
    )
}
